...
import { useSquad } from "../../hooks/useSquad"; // <-- Corrected path
...
export function RecommendationsPage() {
  ...
  return (
    <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', color: '#f8fafc' }}>
      ...
      {/* Strategic Analysis Banner */}
      <div style={{ padding: '1.25rem', backgroundColor: '#1e293b', borderRadius: '12px', border: '1px solid #334155' }}>
        <h3 style={{ margin: 0, fontSize: '1.1rem', color: '#f8fafc', fontWeight: 'bold' }}>Engine Strategic Analysis</h3>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <span style={{ fontSize: '0.9rem', fontWeight: 'bold', color: '#f8fafc' }}>Confidence Score:</span>
          <span style={{ backgroundColor: confidenceScore >= 70 ? '#10b981' : '#059669', color: '#fff', padding: '0.25rem 0.75rem', borderRadius: '6px', fontWeight: 'bold', fontSize: '0.85rem' }}>
            {confidenceScore}%
          </span>
        </div>
      </div>

      {/* Dynamic Positional Cards Grid */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem' }}>
        {rawPlayers.map((slot, index) => {
          ...
          return (
            <div key={slot.id || index} style={{ padding: '1.25rem', backgroundColor: '#1e293b', borderRadius: '12px', border: '1px solid #334155', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
