...
export function recommendForFormation(type, level, formationId, overrides = {}) {
  ...
  const results = uniquePositions.map((position) => {
    // Handle 'GK' slot mapping safely for position scoring engine
    let targetPos = position;
    if (position === 'GK') {
      targetPos = isGoalkeeperType(type) ? 'GK' : 'CB';
    }

    try {
      const scored = scorePosition(type, level, targetPos, overrides);
      return {
        position,
        score: scored.score,
      };
